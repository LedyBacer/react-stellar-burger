...
function Ingredients({item, handleClick, type}: TIngredients) {
    const bunId = useAppSelector(state => state.constructorCart.bunId);
...
function BurgerIngredients() {
    ...
    const refFillers = React.useRef<HTMLDivElement>(null);
    ...
    const openModal = (e: React.MouseEvent<HTMLDivElement>) => {
        const selectedProductId = (e.target as HTMLDivElement).getAttribute("data-id");
    ...
    useEffect(() => {
        ...
        currents.forEach((e) => {
            if (e) {return watcher.observe(e);}
        });
    }, []);

    useEffect(() => {
        if (scroll === "buns" && refBun.current) {
            refBun.current.scrollIntoView({ behavior: "smooth" });
        }
        if (scroll === "sauces" && refSauce.current) {
            refSauce.current.scrollIntoView({ behavior: "smooth" });
        }
        if (scroll === "fillers" && refFillers.current) {
